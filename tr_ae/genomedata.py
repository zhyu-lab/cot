...
class GenomeData:
    ...
    def preprocess_data(self):
        ...
        tv3 = np.logical_and(self.data_map_all > 0.1, self.data_map_all < 0.9)
        ...
        # self.data_rc_all = np.float32(self.data_rc_all)
        self.data_lrc_all = np.float32(np.log2(self.data_rc_all + 1e-5))
        self.data_rc_all = np.float32(self.data_rc_all)
        # median normalization
        # for i in range(self.data_rc_all.shape[1]):
        #     self.data_rc_all[:, i] = self.data_rc_all[:, i] / (np.median(self.data_rc_all[:, i]) + 1e-10)
        # meidian filtering
        # w_size = 100
        # for j in range(self.data_rc_all.shape[0]):
        #     indxs = np.array(range(max(0, j - w_size), min(self.data_rc_all.shape[0], j + w_size)))
        #     m = np.median(self.data_rc_all[indxs, :], axis=0)
        #     tv = self.data_rc_all[j, :] / (m + 1e-8) > 5
        #     self.data_rc_all[j, tv] = m[tv]

        # data_pos_all = (self.data_bin_all - 1) * self.bin_size
        # for i in range(4):
        #     tv = self.data_chr_all == 1
        #     data_pos = data_pos_all[tv]
        #     pyplot.subplot(2, 2, i + 1)
        #     pyplot.plot(data_pos, self.data_lrc_all[tv, i], '.')
        # pyplot.show()
        # for i in range(4):
        #     tv = self.data_chr_all == 2
        #     data_pos = data_pos_all[tv]
        #     pyplot.subplot(2, 2, i + 1)
        #     pyplot.plot(data_pos, self.data_lrc_all[tv, i], '.')
        # pyplot.show()
        # for i in range(4):
        #     tv = self.data_chr_all == 3
        #     data_pos = data_pos_all[tv]
        #     pyplot.subplot(2, 2, i + 1)
        #     pyplot.plot(data_pos, self.data_lrc_all[tv, i], '.')
        # pyplot.show()
    ...
    def map_correction(self):
        m_all_map = np.median(self.data_rc_all, axis=0)
        int_map = np.int8(self.data_map_all * 100)
        int_map_u = np.unique(int_map)
        corrected_rc = np.zeros(self.data_rc_all.shape)
        for i in range(len(int_map_u)):
            tv = int_map == int_map_u[i]
            m_map = np.median(self.data_rc_all[tv,], axis=0)
            corrected_rc[tv,] = self.data_rc_all[tv,] * np.tile(m_all_map, (np.sum(tv), 1)) / (
                        np.tile(m_map, (np.sum(tv), 1)) + 1e-10)
        # pyplot.subplot(2, 1, 1)
        # pyplot.plot(self.data_map_all, self.data_rc_all[:, 1], '.')
        # pyplot.subplot(2, 1, 2)
        # pyplot.plot(self.data_map_all, corrected_rc[:, 1], '.')
        # pyplot.show()
