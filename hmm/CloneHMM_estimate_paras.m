function [LL,prior,transmat,o,sigma,nrIterations] = ...
    CloneHMM_estimate_paras(init_CloneHMM_paras,thresh,max_iter,verbose)
...
prior = init_CloneHMM_paras{1};
transmat = init_CloneHMM_paras{2};
o = init_CloneHMM_paras{3};
sigma = init_CloneHMM_paras{4};

while (num_iter <= max_iter) && ~converged
    % perform EM algorithm
    [loglik,exp_num_trans,exp_num_visits1,o_u,sigma_u] = ...
        CloneHMM_compute_ess(prior,transmat,o,sigma);
    
    converged = em_converged_m(loglik,previous_loglik,verbose,thresh);
    
    % update parameters
    if init_CloneHMM_paras{5}(1)
        prior = norm_trans(exp_num_visits1',0)';
    end
    if init_CloneHMM_paras{5}(2) && ~isempty(exp_num_trans)
        % clamp_thres = 1-1e-4;
        transmat = norm_trans(exp_num_trans,clamp_thres);
    end
    if init_CloneHMM_paras{5}(3) %update o here
        o = o_u;
    end
    if init_CloneHMM_paras{5}(4) %update sigma here
        sigma = sigma_u;
    end
...
for i = 1:num_chrs
    ...
    for c = 1:num_cell
        ...
        [obslik,condi_probs_fluct] = CloneHMM_get_obslik(data_lrc_chr(c,:),o,sigma);
...
%-----------------------M step-----------------------------
%update sigma
sigma_u = CloneHMM_update_sigma(o,sigma);

%update o
o_u = CloneHMM_update_o(o);

end

%--------------------------------------------------------------------------
function sigma_u = CloneHMM_update_sigma(o,sigma)
...
%--------------------------------------------------------------------------
function o_u = CloneHMM_update_o(o)
