function CloneHMM(lrcFile,labelFile,outputDir,maxCN)
% 02/05/2023
...
disp(['CloneHMM (version ' current_version ') is loading...'])

%--------------load data--------------------
[data_lrc_all,data_chr_all,data_bin_all,bin_size,cell_labels] = CloneHMM_load_data(lrcFile,labelFile);
...
%     end
    ...
    init_CloneHMM_paras = [{[]},{[]},{[]},{[]},{[]}]; % initial parameters:pie,transmat,sigma,o
    %------------------ call HMM --------------------
    disp(['----------Call CNAs for cluster ' num2str(c) ' ----------']);
    CloneHMM_paras = CloneHMM_main(init_CloneHMM_paras,thres_EM,max_iter,verbose);
    o = CloneHMM_paras{3}{1};
    sigma = CloneHMM_paras{4}{1};
    data_paras_all(c,:) = [o sigma];
 
    [p_states,aCN,segments_all] = CloneHMM_process_results();
